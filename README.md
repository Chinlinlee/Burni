...
- `npm run search-parameter:build-artifacts` regenerates the committed runtime compile artifact at `models/FHIR/searchParameter/registry/artifacts/compiled-builtin-definitions.json` and the migration artifacts (lookup matrix, example mapping, fixture archive, hit-set, migration manifest, and resource-enablement). Run it when the official SearchParameter Bundle, compiler behavior, type maps under `api_generator/to-code-use-definition`, or fixture corpus changes. Set `FHIR_EXAMPLES_DIR` to rediscover official examples during a rebuild. Default registry startup hydrates this compile artifact and rejects application readiness when it is missing or stale.
...
The committed canonical source is the FHIR R4/4.0.1 SearchParameter Bundle. Legacy inventory files are not runtime inputs and are not regenerated by these commands. `npm run build` does not regenerate the SearchParameter compile artifact; run `search-parameter:build-artifacts` separately when the inputs above change.

### MongoDB provisioning commands
