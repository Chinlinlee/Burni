...
- [ ] 5.3 建立每個 compiled lookup 的 positive hit-set、companion negative hit-set 與適用的 `:missing` 測試。
- [ ] 5.4 建立 choice、union、nested array、reference correlation、synthetic fixture 與 no-lookup resource 的 Mongo integration tests。
- [ ] 5.5 以 compatibility-plus-corrections 驗證 public search 結果，不以 legacy filter JSON equality 作為 gate。
