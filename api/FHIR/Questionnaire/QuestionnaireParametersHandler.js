...
//#region code
paramsSearchFields["code"] = ["item.code"];
const codeSearchFunc = {};
codeSearchFunc["item.code"] = (item, field) => {
...
//#endregion
//#region subject-type
paramsSearchFields["subject-type"] = ["subjectType"];
const subject_typeSearchFunc = {};
subject_typeSearchFunc["subjectType"] = (item, field) => {
    return queryBuild.tokenQuery(item, "", field, "", false);
};

paramsSearch["subject-type"] = (query) => {
    try {
        queryHandler.getPolyTokenQuery(query, paramsSearchFields, "subject-type", subject_typeSearchFunc);
    } catch (e) {
        console.error(e);
        throw e;
    }
