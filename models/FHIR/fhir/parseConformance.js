...
class ParseConformance {
            ...
            for (let x in structureDefinition.snapshot.element) {
                ...
                if (element.type.length === 1 && element.type[0].code !== 'Reference' && !elementId.includes('[x]') && !elementId.includes(':')) {
                    ...
                    if (elementId.endsWith("[x]")) {
                        ...
                        let existProperty = parsedStructureDefinition._properties.find(v => v._name == elementId);
                        this.populateValueSet(element, existProperty, true)
                    }
                }
            }
        }
    ...
    populateValueSet(element, property, isSlicing = false) {
        if (!Array.isArray(property._valueSetList)) {
            property._valueSetList = [];
        }
        let valueSet = {
            _valueSet: "",
            _valueSetStrength: "",
        ...
        if (element.binding) {
            ...
            if (this.version === fhir_1.Versions.R4 && binding.valueSet) {
                ...
                valueSet._valueSet = binding.valueSetReference.reference;
