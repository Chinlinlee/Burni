...
module.exports = function(connection = mongoose) {
    ...
    ImmunizationSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.model("Immunization_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    ImmunizationSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb.model("Immunization_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Immunization/${item.id}/_history/${version}`
    ...
    ImmunizationSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await modelConnection.model("Immunization_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    ImmunizationSchema.pre('findOneAndDelete', async function(next) {
        ...
        let createdDocs = await modelConnection.model("Immunization_history").create(item);
        next();
    });
    ...
    const ImmunizationModel = modelConnection.model("Immunization", ImmunizationSchema, "Immunization");
