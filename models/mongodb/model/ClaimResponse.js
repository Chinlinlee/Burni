...
module.exports = function() {
    ...
    ClaimResponseSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.ClaimResponse_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    ClaimResponseSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['ClaimResponse_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/ClaimResponse/${item.id}/_history/${version}`
    ...
    ClaimResponseSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['ClaimResponse_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const ClaimResponseModel = mongoose.model("ClaimResponse", ClaimResponseSchema, "ClaimResponse");
