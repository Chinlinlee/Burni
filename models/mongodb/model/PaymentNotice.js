...
module.exports = function() {
    ...
    module.exports.schema = PaymentNotice;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const PaymentNoticeSchema = new mongoose.Schema(PaymentNotice, schemaConfig);


    PaymentNoticeSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    PaymentNoticeSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/PaymentNotice/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
    ...
    PaymentNoticeSchema.pre('findOneAndDelete', async function(next) {
