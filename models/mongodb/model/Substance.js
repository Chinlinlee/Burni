...
module.exports = function() {
    ...
    SubstanceSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.Substance_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    SubstanceSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['Substance_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Substance/${item.id}/_history/${version}`
    ...
    SubstanceSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['Substance_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const SubstanceModel = mongoose.model("Substance", SubstanceSchema, "Substance");
