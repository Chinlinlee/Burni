...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = Substance;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const SubstanceSchema = new mongoose.Schema(Substance, schemaConfig);


    SubstanceSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    SubstanceSchema.pre('save', async function(next) {
        const mongodb = modelConnection;
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            let storedID = await mongodb.model("FHIRStoredID").findOne({
                id: this.id
            });
            if (storedID.resourceType != "Substance") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("Substance_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    SubstanceSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Substance/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("Substance_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Substance/${item.id}/_history/${version}`
    ...
    SubstanceSchema.pre('findOneAndUpdate', async function(next) {
    ...
    SubstanceSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("Substance_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    SubstanceSchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in Substance resource`);
        }
        ...
        let createdDocs = await modelConnection.model("Substance_history").create(item);
        next();
    });

    SubstanceSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const SubstanceModel = modelConnection.model("Substance", SubstanceSchema, "Substance");
