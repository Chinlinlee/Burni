...
module.exports = function() {
    ...
    GoalSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.Goal_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    GoalSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['Goal_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Goal/${item.id}/_history/${version}`
    ...
    GoalSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['Goal_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const GoalModel = mongoose.model("Goal", GoalSchema, "Goal");
    return GoalModel;
};