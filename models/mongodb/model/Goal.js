...
module.exports = function(connection = mongoose) {
    ...
    const GoalSchema = new mongoose.Schema(Goal, schemaConfig);


    GoalSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    GoalSchema.pre('save', async function(next) {
        ...
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            ...
            if (storedID.resourceType != "Goal") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("Goal_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    GoalSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Goal/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("Goal_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Goal/${item.id}/_history/${version}`
    ...
    GoalSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("Goal_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    GoalSchema.pre('findOneAndDelete', async function(next) {
        ...
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in Goal resource`);
        }
        ...
        let createdDocs = await modelConnection.model("Goal_history").create(item);
        next();
    });

    GoalSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const GoalModel = modelConnection.model("Goal", GoalSchema, "Goal");
    return GoalModel;
};