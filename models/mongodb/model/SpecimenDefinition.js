...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = SpecimenDefinition;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const SpecimenDefinitionSchema = new mongoose.Schema(SpecimenDefinition, schemaConfig);


    SpecimenDefinitionSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    SpecimenDefinitionSchema.pre('save', async function(next) {
        const mongodb = modelConnection;
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            let storedID = await mongodb.model("FHIRStoredID").findOne({
                id: this.id
            });
            if (storedID.resourceType != "SpecimenDefinition") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("SpecimenDefinition_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    SpecimenDefinitionSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/SpecimenDefinition/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("SpecimenDefinition_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/SpecimenDefinition/${item.id}/_history/${version}`
    ...
    SpecimenDefinitionSchema.pre('findOneAndUpdate', async function(next) {
    ...
    SpecimenDefinitionSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("SpecimenDefinition_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    SpecimenDefinitionSchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in SpecimenDefinition resource`);
        }
        ...
        let createdDocs = await modelConnection.model("SpecimenDefinition_history").create(item);
        next();
    });

    SpecimenDefinitionSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const SpecimenDefinitionModel = modelConnection.model("SpecimenDefinition", SpecimenDefinitionSchema, "SpecimenDefinition");
    return SpecimenDefinitionModel;
};