...
module.exports = function() {
    ...
    ContractSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.Contract_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    ContractSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['Contract_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Contract/${item.id}/_history/${version}`
    ...
    ContractSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['Contract_history'].create(item);
        } catch (e) {
            console.error(e);
        }
