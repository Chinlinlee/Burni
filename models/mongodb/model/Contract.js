...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = Contract;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const ContractSchema = new mongoose.Schema(Contract, schemaConfig);


    ContractSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    ContractSchema.pre('save', async function(next) {
        ...
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            ...
            if (storedID.resourceType != "Contract") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("Contract_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    ContractSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Contract/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("Contract_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Contract/${item.id}/_history/${version}`
    ...
    ContractSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("Contract_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    ContractSchema.pre('findOneAndDelete', async function(next) {
        ...
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in Contract resource`);
        }
        ...
        let createdDocs = await modelConnection.model("Contract_history").create(item);
        next();
    });

    ContractSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const ContractModel = modelConnection.model("Contract", ContractSchema, "Contract");
