...
module.exports = function(connection = mongoose) {
    ...
    AccountSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.model("Account_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    AccountSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb.model("Account_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Account/${item.id}/_history/${version}`
    ...
    AccountSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await modelConnection.model("Account_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    AccountSchema.pre('findOneAndDelete', async function(next) {
        ...
        let createdDocs = await modelConnection.model("Account_history").create(item);
        next();
    });
    ...
    const AccountModel = modelConnection.model("Account", AccountSchema, "Account");
    return AccountModel;
};