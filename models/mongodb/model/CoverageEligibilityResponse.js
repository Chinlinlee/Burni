...
module.exports = function(connection = mongoose) {
    ...
    CoverageEligibilityResponseSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.model("CoverageEligibilityResponse_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    CoverageEligibilityResponseSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb.model("CoverageEligibilityResponse_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/CoverageEligibilityResponse/${item.id}/_history/${version}`
    ...
    CoverageEligibilityResponseSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await modelConnection.model("CoverageEligibilityResponse_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    CoverageEligibilityResponseSchema.pre('findOneAndDelete', async function(next) {
        ...
        let createdDocs = await modelConnection.model("CoverageEligibilityResponse_history").create(item);
        next();
    });
    ...
    const CoverageEligibilityResponseModel = modelConnection.model("CoverageEligibilityResponse", CoverageEligibilityResponseSchema, "CoverageEligibilityResponse");
