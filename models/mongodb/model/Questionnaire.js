...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = Questionnaire;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const QuestionnaireSchema = new mongoose.Schema(Questionnaire, schemaConfig);


    QuestionnaireSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    QuestionnaireSchema.pre('save', async function(next) {
        ...
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            ...
            if (storedID.resourceType != "Questionnaire") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("Questionnaire_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    QuestionnaireSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Questionnaire/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("Questionnaire_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Questionnaire/${item.id}/_history/${version}`
    ...
    QuestionnaireSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("Questionnaire_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    QuestionnaireSchema.pre('findOneAndDelete', async function(next) {
        ...
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in Questionnaire resource`);
        }
        ...
        let createdDocs = await modelConnection.model("Questionnaire_history").create(item);
        next();
    });

    QuestionnaireSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const QuestionnaireModel = modelConnection.model("Questionnaire", QuestionnaireSchema, "Questionnaire");
