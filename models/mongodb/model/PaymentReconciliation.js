...
module.exports = function(connection = mongoose) {
    ...
    PaymentReconciliationSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.model("PaymentReconciliation_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    PaymentReconciliationSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb.model("PaymentReconciliation_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/PaymentReconciliation/${item.id}/_history/${version}`
    ...
    PaymentReconciliationSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await modelConnection.model("PaymentReconciliation_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    PaymentReconciliationSchema.pre('findOneAndDelete', async function(next) {
        ...
        let createdDocs = await modelConnection.model("PaymentReconciliation_history").create(item);
        next();
    });
    ...
    const PaymentReconciliationModel = modelConnection.model("PaymentReconciliation", PaymentReconciliationSchema, "PaymentReconciliation");
