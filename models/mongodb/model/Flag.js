...
module.exports = function(connection = mongoose) {
    ...
    const FlagSchema = new mongoose.Schema(Flag, schemaConfig);


    FlagSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    FlagSchema.pre('save', async function(next) {
        ...
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            ...
            if (storedID.resourceType != "Flag") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("Flag_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    FlagSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Flag/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("Flag_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Flag/${item.id}/_history/${version}`
    ...
    FlagSchema.pre('findOneAndUpdate', async function(next) {
    ...
    FlagSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("Flag_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    FlagSchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in Flag resource`);
        }
        ...
        let createdDocs = await modelConnection.model("Flag_history").create(item);
        next();
    });

    FlagSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const FlagModel = modelConnection.model("Flag", FlagSchema, "Flag");
    return FlagModel;
};