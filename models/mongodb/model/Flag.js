...
module.exports = function() {
    ...
    FlagSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.Flag_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    FlagSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['Flag_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Flag/${item.id}/_history/${version}`
    ...
    FlagSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['Flag_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const FlagModel = mongoose.model("Flag", FlagSchema, "Flag");
    return FlagModel;
};