...
module.exports = function() {
    ...
    SubstanceSpecificationSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.SubstanceSpecification_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    SubstanceSpecificationSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['SubstanceSpecification_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/SubstanceSpecification/${item.id}/_history/${version}`
    ...
    SubstanceSpecificationSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['SubstanceSpecification_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const SubstanceSpecificationModel = mongoose.model("SubstanceSpecification", SubstanceSpecificationSchema, "SubstanceSpecification");
