...
module.exports = function(connection = mongoose) {
    ...
    ExplanationOfBenefitSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.model("ExplanationOfBenefit_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    ExplanationOfBenefitSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb.model("ExplanationOfBenefit_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/ExplanationOfBenefit/${item.id}/_history/${version}`
    ...
    ExplanationOfBenefitSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await modelConnection.model("ExplanationOfBenefit_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    ExplanationOfBenefitSchema.pre('findOneAndDelete', async function(next) {
        ...
        let createdDocs = await modelConnection.model("ExplanationOfBenefit_history").create(item);
        next();
    });
    ...
    const ExplanationOfBenefitModel = modelConnection.model("ExplanationOfBenefit", ExplanationOfBenefitSchema, "ExplanationOfBenefit");
