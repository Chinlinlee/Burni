...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = EnrollmentRequest;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const EnrollmentRequestSchema = new mongoose.Schema(EnrollmentRequest, schemaConfig);


    EnrollmentRequestSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    EnrollmentRequestSchema.pre('save', async function(next) {
        ...
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            ...
            if (storedID.resourceType != "EnrollmentRequest") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("EnrollmentRequest_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    EnrollmentRequestSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/EnrollmentRequest/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("EnrollmentRequest_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/EnrollmentRequest/${item.id}/_history/${version}`
    ...
    EnrollmentRequestSchema.pre('findOneAndUpdate', async function(next) {
    ...
    EnrollmentRequestSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("EnrollmentRequest_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    EnrollmentRequestSchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in EnrollmentRequest resource`);
        }
        ...
        let createdDocs = await modelConnection.model("EnrollmentRequest_history").create(item);
        next();
    });

    EnrollmentRequestSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const EnrollmentRequestModel = modelConnection.model("EnrollmentRequest", EnrollmentRequestSchema, "EnrollmentRequest");
