...
module.exports = function(connection = mongoose) {
    ...
    MedicinalProductPharmaceuticalSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.model("MedicinalProductPharmaceutical_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    MedicinalProductPharmaceuticalSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb.model("MedicinalProductPharmaceutical_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/MedicinalProductPharmaceutical/${item.id}/_history/${version}`
    ...
    MedicinalProductPharmaceuticalSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await modelConnection.model("MedicinalProductPharmaceutical_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    MedicinalProductPharmaceuticalSchema.pre('findOneAndDelete', async function(next) {
        ...
        let createdDocs = await modelConnection.model("MedicinalProductPharmaceutical_history").create(item);
        next();
    });
    ...
    const MedicinalProductPharmaceuticalModel = modelConnection.model("MedicinalProductPharmaceutical", MedicinalProductPharmaceuticalSchema, "MedicinalProductPharmaceutical");
