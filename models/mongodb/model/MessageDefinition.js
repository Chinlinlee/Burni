...
module.exports = function(connection = mongoose) {
    ...
    MessageDefinitionSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.model("MessageDefinition_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    MessageDefinitionSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb.model("MessageDefinition_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/MessageDefinition/${item.id}/_history/${version}`
    ...
    MessageDefinitionSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await modelConnection.model("MessageDefinition_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    MessageDefinitionSchema.pre('findOneAndDelete', async function(next) {
        ...
        let createdDocs = await modelConnection.model("MessageDefinition_history").create(item);
        next();
    });
    ...
    const MessageDefinitionModel = modelConnection.model("MessageDefinition", MessageDefinitionSchema, "MessageDefinition");
