...
module.exports = function() {
    ...
    DeviceRequestSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.DeviceRequest_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    DeviceRequestSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['DeviceRequest_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/DeviceRequest/${item.id}/_history/${version}`
    ...
    DeviceRequestSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['DeviceRequest_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const DeviceRequestModel = mongoose.model("DeviceRequest", DeviceRequestSchema, "DeviceRequest");
