...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = Procedure;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const ProcedureSchema = new mongoose.Schema(Procedure, schemaConfig);


    ProcedureSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    ProcedureSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Procedure/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
    ...
    ProcedureSchema.pre('findOneAndDelete', async function(next) {
