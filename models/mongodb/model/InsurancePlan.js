...
module.exports = function() {
    ...
    InsurancePlanSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.InsurancePlan_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    InsurancePlanSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['InsurancePlan_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/InsurancePlan/${item.id}/_history/${version}`
    ...
    InsurancePlanSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['InsurancePlan_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const InsurancePlanModel = mongoose.model("InsurancePlan", InsurancePlanSchema, "InsurancePlan");
