...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = Communication;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const CommunicationSchema = new mongoose.Schema(Communication, schemaConfig);


    CommunicationSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    CommunicationSchema.pre('save', async function(next) {
        ...
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            ...
            if (storedID.resourceType != "Communication") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("Communication_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    CommunicationSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Communication/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("Communication_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Communication/${item.id}/_history/${version}`
    ...
    CommunicationSchema.pre('findOneAndUpdate', async function(next) {
    ...
    CommunicationSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("Communication_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    CommunicationSchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in Communication resource`);
        }
        ...
        let createdDocs = await modelConnection.model("Communication_history").create(item);
        next();
    });

    CommunicationSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const CommunicationModel = modelConnection.model("Communication", CommunicationSchema, "Communication");
