...
module.exports = function() {
    ...
    CommunicationSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.Communication_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    CommunicationSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['Communication_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Communication/${item.id}/_history/${version}`
    ...
    CommunicationSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['Communication_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const CommunicationModel = mongoose.model("Communication", CommunicationSchema, "Communication");
