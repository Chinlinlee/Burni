...
module.exports = function() {
    ...
    CodeSystemSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.CodeSystem_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    CodeSystemSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['CodeSystem_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/CodeSystem/${item.id}/_history/${version}`
    ...
    CodeSystemSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['CodeSystem_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const CodeSystemModel = mongoose.model("CodeSystem", CodeSystemSchema, "CodeSystem");
