...
module.exports = function(connection = mongoose) {
    ...
    const OrganizationSchema = new mongoose.Schema(Organization, schemaConfig);
    ...
    OrganizationSchema.pre('save', async function(next) {
        ...
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            ...
            if (storedID.resourceType != "Organization") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("Organization_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    OrganizationSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Organization/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("Organization_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Organization/${item.id}/_history/${version}`
    ...
    OrganizationSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await modelConnection.model("Organization_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    OrganizationSchema.pre('findOneAndDelete', async function(next) {
        ...
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in Organization resource`);
        }
        ...
        let createdDocs = await modelConnection.model("Organization_history").create(item);
        next();
    });

    OrganizationSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const OrganizationModel = modelConnection.model("Organization", OrganizationSchema, "Organization");
