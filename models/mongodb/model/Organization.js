...
module.exports = function() {
    ...
    OrganizationSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.Organization_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    OrganizationSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['Organization_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Organization/${item.id}/_history/${version}`
    ...
    OrganizationSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['Organization_history'].create(item);
        } catch (e) {
            console.error(e);
        }
