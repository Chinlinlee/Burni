...
module.exports = function() {
    ...
    EnrollmentResponseSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.EnrollmentResponse_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    EnrollmentResponseSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['EnrollmentResponse_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/EnrollmentResponse/${item.id}/_history/${version}`
    ...
    EnrollmentResponseSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['EnrollmentResponse_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const EnrollmentResponseModel = mongoose.model("EnrollmentResponse", EnrollmentResponseSchema, "EnrollmentResponse");
