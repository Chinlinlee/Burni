...
module.exports = function(connection = mongoose) {
    ...
    const LinkageSchema = new mongoose.Schema(Linkage, schemaConfig);


    LinkageSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    LinkageSchema.pre('save', async function(next) {
        const mongodb = modelConnection;
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            let storedID = await mongodb.model("FHIRStoredID").findOne({
                id: this.id
            });
            if (storedID.resourceType != "Linkage") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("Linkage_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    LinkageSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Linkage/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("Linkage_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Linkage/${item.id}/_history/${version}`
    ...
    LinkageSchema.pre('findOneAndUpdate', async function(next) {
    ...
    LinkageSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("Linkage_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    LinkageSchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in Linkage resource`);
        }
        ...
        let createdDocs = await modelConnection.model("Linkage_history").create(item);
        next();
    });

    LinkageSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const LinkageModel = modelConnection.model("Linkage", LinkageSchema, "Linkage");
    return LinkageModel;
};