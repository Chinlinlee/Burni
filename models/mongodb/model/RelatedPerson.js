...
module.exports = function(connection = mongoose) {
    ...
    RelatedPersonSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.model("RelatedPerson_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    RelatedPersonSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb.model("RelatedPerson_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/RelatedPerson/${item.id}/_history/${version}`
    ...
    RelatedPersonSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await modelConnection.model("RelatedPerson_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    RelatedPersonSchema.pre('findOneAndDelete', async function(next) {
        ...
        let createdDocs = await modelConnection.model("RelatedPerson_history").create(item);
        next();
    });
    ...
    const RelatedPersonModel = modelConnection.model("RelatedPerson", RelatedPersonSchema, "RelatedPerson");
