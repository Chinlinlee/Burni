...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = Condition;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const ConditionSchema = new mongoose.Schema(Condition, schemaConfig);


    ConditionSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    ConditionSchema.pre('save', async function(next) {
        ...
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            ...
            if (storedID.resourceType != "Condition") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("Condition_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    ConditionSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Condition/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("Condition_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Condition/${item.id}/_history/${version}`
    ...
    ConditionSchema.pre('findOneAndUpdate', async function(next) {
    ...
    ConditionSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("Condition_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    ConditionSchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in Condition resource`);
        }
        ...
        let createdDocs = await modelConnection.model("Condition_history").create(item);
        next();
    });

    ConditionSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const ConditionModel = modelConnection.model("Condition", ConditionSchema, "Condition");
