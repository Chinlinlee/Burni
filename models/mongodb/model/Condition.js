...
module.exports = function() {
    ...
    ConditionSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.Condition_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    ConditionSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['Condition_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Condition/${item.id}/_history/${version}`
    ...
    ConditionSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['Condition_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const ConditionModel = mongoose.model("Condition", ConditionSchema, "Condition");
