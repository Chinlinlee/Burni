...
module.exports = function() {
    ...
    module.exports.schema = Encounter;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const EncounterSchema = new mongoose.Schema(Encounter, schemaConfig);


    EncounterSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    EncounterSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Encounter/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
    ...
    EncounterSchema.pre('findOneAndDelete', async function(next) {
