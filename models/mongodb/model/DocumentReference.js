...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = DocumentReference;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const DocumentReferenceSchema = new mongoose.Schema(DocumentReference, schemaConfig);


    DocumentReferenceSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    DocumentReferenceSchema.pre('save', async function(next) {
        const mongodb = modelConnection;
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            let storedID = await mongodb.model("FHIRStoredID").findOne({
                id: this.id
            });
            if (storedID.resourceType != "DocumentReference") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("DocumentReference_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    DocumentReferenceSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            setHistoryProvenance(item, {
                method: "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/DocumentReference/${item.id}/_history/${version}`,
                status: "201"
            });
            let createdDocs = await mongodb.model("DocumentReference_history").create(item);
        } else {
            setHistoryProvenance(item, {
                method: "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/DocumentReference/${item.id}/_history/${version}`,
    ...
    DocumentReferenceSchema.pre('findOneAndUpdate', async function(next) {
    ...
    DocumentReferenceSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("DocumentReference_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    DocumentReferenceSchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in DocumentReference resource`);
        }
        ...
        let createdDocs = await modelConnection.model("DocumentReference_history").create(item);
        next();
    });

    DocumentReferenceSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const DocumentReferenceModel = modelConnection.model("DocumentReference", DocumentReferenceSchema, "DocumentReference");
