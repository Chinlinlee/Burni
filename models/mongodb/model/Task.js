...
module.exports = function() {
    ...
    TaskSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.Task_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    TaskSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['Task_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Task/${item.id}/_history/${version}`
    ...
    TaskSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['Task_history'].create(item);
        } catch (e) {
            console.error(e);
        }
