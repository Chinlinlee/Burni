...
module.exports = function(connection = mongoose) {
    ...
    VerificationResultSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.model("VerificationResult_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    VerificationResultSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb.model("VerificationResult_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/VerificationResult/${item.id}/_history/${version}`
    ...
    VerificationResultSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await modelConnection.model("VerificationResult_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    VerificationResultSchema.pre('findOneAndDelete', async function(next) {
        ...
        let createdDocs = await modelConnection.model("VerificationResult_history").create(item);
        next();
    });
    ...
    const VerificationResultModel = modelConnection.model("VerificationResult", VerificationResultSchema, "VerificationResult");
