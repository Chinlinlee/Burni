...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = GuidanceResponse;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const GuidanceResponseSchema = new mongoose.Schema(GuidanceResponse, schemaConfig);


    GuidanceResponseSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    GuidanceResponseSchema.pre('save', async function(next) {
        const mongodb = modelConnection;
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            let storedID = await mongodb.model("FHIRStoredID").findOne({
                id: this.id
            });
            if (storedID.resourceType != "GuidanceResponse") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("GuidanceResponse_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    GuidanceResponseSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/GuidanceResponse/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("GuidanceResponse_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/GuidanceResponse/${item.id}/_history/${version}`
    ...
    GuidanceResponseSchema.pre('findOneAndUpdate', async function(next) {
    ...
    GuidanceResponseSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("GuidanceResponse_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    GuidanceResponseSchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in GuidanceResponse resource`);
        }
        ...
        let createdDocs = await modelConnection.model("GuidanceResponse_history").create(item);
        next();
    });

    GuidanceResponseSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const GuidanceResponseModel = modelConnection.model("GuidanceResponse", GuidanceResponseSchema, "GuidanceResponse");
