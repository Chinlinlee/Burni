...
module.exports = function() {
    ...
    GuidanceResponseSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.GuidanceResponse_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    GuidanceResponseSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['GuidanceResponse_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/GuidanceResponse/${item.id}/_history/${version}`
    ...
    GuidanceResponseSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['GuidanceResponse_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const GuidanceResponseModel = mongoose.model("GuidanceResponse", GuidanceResponseSchema, "GuidanceResponse");
