...
module.exports = function() {
    ...
    SubstancePolymerSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.SubstancePolymer_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    SubstancePolymerSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['SubstancePolymer_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/SubstancePolymer/${item.id}/_history/${version}`
    ...
    SubstancePolymerSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['SubstancePolymer_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const SubstancePolymerModel = mongoose.model("SubstancePolymer", SubstancePolymerSchema, "SubstancePolymer");
