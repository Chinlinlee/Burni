...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = SubstancePolymer;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const SubstancePolymerSchema = new mongoose.Schema(SubstancePolymer, schemaConfig);


    SubstancePolymerSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    SubstancePolymerSchema.pre('save', async function(next) {
        const mongodb = modelConnection;
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            let storedID = await mongodb.model("FHIRStoredID").findOne({
                id: this.id
            });
            if (storedID.resourceType != "SubstancePolymer") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("SubstancePolymer_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    SubstancePolymerSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/SubstancePolymer/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("SubstancePolymer_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/SubstancePolymer/${item.id}/_history/${version}`
    ...
    SubstancePolymerSchema.pre('findOneAndUpdate', async function(next) {
    ...
    SubstancePolymerSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("SubstancePolymer_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    SubstancePolymerSchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in SubstancePolymer resource`);
        }
        ...
        let createdDocs = await modelConnection.model("SubstancePolymer_history").create(item);
        next();
    });

    SubstancePolymerSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const SubstancePolymerModel = modelConnection.model("SubstancePolymer", SubstancePolymerSchema, "SubstancePolymer");
    return SubstancePolymerModel;
};