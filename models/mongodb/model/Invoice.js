...
module.exports = function() {
    ...
    InvoiceSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.Invoice_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    InvoiceSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['Invoice_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Invoice/${item.id}/_history/${version}`
    ...
    InvoiceSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['Invoice_history'].create(item);
        } catch (e) {
            console.error(e);
        }
