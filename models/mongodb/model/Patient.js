...
module.exports = function(connection = mongoose) {
    ...
    PatientSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.model("Patient_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    PatientSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb.model("Patient_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Patient/${item.id}/_history/${version}`
    ...
    PatientSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await modelConnection.model("Patient_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    PatientSchema.pre('findOneAndDelete', async function(next) {
        ...
        let createdDocs = await modelConnection.model("Patient_history").create(item);
        next();
    });
    ...
    const PatientModel = modelConnection.model("Patient", PatientSchema, "Patient");
    return PatientModel;
};