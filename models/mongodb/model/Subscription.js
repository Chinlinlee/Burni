...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = Subscription;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const SubscriptionSchema = new mongoose.Schema(Subscription, schemaConfig);


    SubscriptionSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    SubscriptionSchema.pre('save', async function(next) {
        ...
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            ...
            if (storedID.resourceType != "Subscription") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("Subscription_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    SubscriptionSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Subscription/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("Subscription_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Subscription/${item.id}/_history/${version}`
    ...
    SubscriptionSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("Subscription_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    SubscriptionSchema.pre('findOneAndDelete', async function(next) {
        ...
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in Subscription resource`);
        }
        ...
        let createdDocs = await modelConnection.model("Subscription_history").create(item);
        next();
    });

    SubscriptionSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const SubscriptionModel = modelConnection.model("Subscription", SubscriptionSchema, "Subscription");
