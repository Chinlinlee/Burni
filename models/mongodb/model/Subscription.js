...
module.exports = function() {
    ...
    SubscriptionSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.Subscription_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    SubscriptionSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['Subscription_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Subscription/${item.id}/_history/${version}`
    ...
    SubscriptionSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['Subscription_history'].create(item);
        } catch (e) {
            console.error(e);
        }
