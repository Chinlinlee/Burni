...
module.exports = function() {
    ...
    StructureMapSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.StructureMap_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    StructureMapSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['StructureMap_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/StructureMap/${item.id}/_history/${version}`
    ...
    StructureMapSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['StructureMap_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const StructureMapModel = mongoose.model("StructureMap", StructureMapSchema, "StructureMap");
