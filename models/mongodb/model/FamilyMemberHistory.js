...
module.exports = function(connection = mongoose) {
    ...
    FamilyMemberHistorySchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.model("FamilyMemberHistory_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    FamilyMemberHistorySchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb.model("FamilyMemberHistory_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/FamilyMemberHistory/${item.id}/_history/${version}`
    ...
    FamilyMemberHistorySchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await modelConnection.model("FamilyMemberHistory_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    FamilyMemberHistorySchema.pre('findOneAndDelete', async function(next) {
        ...
        let createdDocs = await modelConnection.model("FamilyMemberHistory_history").create(item);
        next();
    });
    ...
    const FamilyMemberHistoryModel = modelConnection.model("FamilyMemberHistory", FamilyMemberHistorySchema, "FamilyMemberHistory");
