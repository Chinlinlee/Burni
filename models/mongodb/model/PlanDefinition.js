...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = PlanDefinition;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const PlanDefinitionSchema = new mongoose.Schema(PlanDefinition, schemaConfig);


    PlanDefinitionSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    PlanDefinitionSchema.pre('save', async function(next) {
        ...
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            ...
            if (storedID.resourceType != "PlanDefinition") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("PlanDefinition_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    PlanDefinitionSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/PlanDefinition/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("PlanDefinition_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/PlanDefinition/${item.id}/_history/${version}`
    ...
    PlanDefinitionSchema.pre('findOneAndUpdate', async function(next) {
    ...
    PlanDefinitionSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("PlanDefinition_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    PlanDefinitionSchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in PlanDefinition resource`);
        }
        ...
        let createdDocs = await modelConnection.model("PlanDefinition_history").create(item);
        next();
    });

    PlanDefinitionSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const PlanDefinitionModel = modelConnection.model("PlanDefinition", PlanDefinitionSchema, "PlanDefinition");
