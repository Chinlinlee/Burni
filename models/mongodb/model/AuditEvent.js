...
module.exports = function() {
    ...
    module.exports.schema = AuditEvent;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const AuditEventSchema = new mongoose.Schema(AuditEvent, schemaConfig);


    AuditEventSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    AuditEventSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/AuditEvent/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
    ...
    AuditEventSchema.pre('findOneAndDelete', async function(next) {
