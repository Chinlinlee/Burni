...
module.exports = function() {
    ...
    module.exports.schema = VisionPrescription;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const VisionPrescriptionSchema = new mongoose.Schema(VisionPrescription, schemaConfig);


    VisionPrescriptionSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    VisionPrescriptionSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/VisionPrescription/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
    ...
    VisionPrescriptionSchema.pre('findOneAndDelete', async function(next) {
