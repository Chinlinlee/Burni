...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = CarePlan;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const CarePlanSchema = new mongoose.Schema(CarePlan, schemaConfig);


    CarePlanSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    CarePlanSchema.pre('save', async function(next) {
        ...
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            ...
            if (storedID.resourceType != "CarePlan") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("CarePlan_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    CarePlanSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/CarePlan/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("CarePlan_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/CarePlan/${item.id}/_history/${version}`
    ...
    CarePlanSchema.pre('findOneAndUpdate', async function(next) {
    ...
    CarePlanSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("CarePlan_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    CarePlanSchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in CarePlan resource`);
        }
        ...
        let createdDocs = await modelConnection.model("CarePlan_history").create(item);
        next();
    });

    CarePlanSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const CarePlanModel = modelConnection.model("CarePlan", CarePlanSchema, "CarePlan");
