...
module.exports = function() {
    ...
    CarePlanSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.CarePlan_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    CarePlanSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['CarePlan_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/CarePlan/${item.id}/_history/${version}`
    ...
    CarePlanSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['CarePlan_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const CarePlanModel = mongoose.model("CarePlan", CarePlanSchema, "CarePlan");
