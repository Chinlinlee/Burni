...
module.exports = function() {
    ...
    module.exports.schema = MedicinalProductIngredient;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const MedicinalProductIngredientSchema = new mongoose.Schema(MedicinalProductIngredient, schemaConfig);


    MedicinalProductIngredientSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    MedicinalProductIngredientSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/MedicinalProductIngredient/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
    ...
    MedicinalProductIngredientSchema.pre('findOneAndDelete', async function(next) {
