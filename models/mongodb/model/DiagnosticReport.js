...
module.exports = function(connection = mongoose) {
    ...
    DiagnosticReportSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.model("DiagnosticReport_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    DiagnosticReportSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb.model("DiagnosticReport_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/DiagnosticReport/${item.id}/_history/${version}`
    ...
    DiagnosticReportSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await modelConnection.model("DiagnosticReport_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    DiagnosticReportSchema.pre('findOneAndDelete', async function(next) {
        ...
        let createdDocs = await modelConnection.model("DiagnosticReport_history").create(item);
        next();
    });
    ...
    const DiagnosticReportModel = modelConnection.model("DiagnosticReport", DiagnosticReportSchema, "DiagnosticReport");
