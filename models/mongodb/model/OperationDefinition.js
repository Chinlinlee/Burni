...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = OperationDefinition;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const OperationDefinitionSchema = new mongoose.Schema(OperationDefinition, schemaConfig);


    OperationDefinitionSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    OperationDefinitionSchema.pre('save', async function(next) {
        const mongodb = modelConnection;
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            let storedID = await mongodb.model("FHIRStoredID").findOne({
                id: this.id
            });
            if (storedID.resourceType != "OperationDefinition") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("OperationDefinition_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    OperationDefinitionSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/OperationDefinition/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("OperationDefinition_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/OperationDefinition/${item.id}/_history/${version}`
    ...
    OperationDefinitionSchema.pre('findOneAndUpdate', async function(next) {
    ...
    OperationDefinitionSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("OperationDefinition_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    OperationDefinitionSchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in OperationDefinition resource`);
        }
        ...
        let createdDocs = await modelConnection.model("OperationDefinition_history").create(item);
        next();
    });

    OperationDefinitionSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const OperationDefinitionModel = modelConnection.model("OperationDefinition", OperationDefinitionSchema, "OperationDefinition");
