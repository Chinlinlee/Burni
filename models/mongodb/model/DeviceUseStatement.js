...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = DeviceUseStatement;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const DeviceUseStatementSchema = new mongoose.Schema(DeviceUseStatement, schemaConfig);


    DeviceUseStatementSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    DeviceUseStatementSchema.pre('save', async function(next) {
        const mongodb = modelConnection;
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            let storedID = await mongodb.model("FHIRStoredID").findOne({
                id: this.id
            });
            if (storedID.resourceType != "DeviceUseStatement") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("DeviceUseStatement_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    DeviceUseStatementSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/DeviceUseStatement/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("DeviceUseStatement_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/DeviceUseStatement/${item.id}/_history/${version}`
    ...
    DeviceUseStatementSchema.pre('findOneAndUpdate', async function(next) {
    ...
    DeviceUseStatementSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("DeviceUseStatement_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    DeviceUseStatementSchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in DeviceUseStatement resource`);
        }
        ...
        let createdDocs = await modelConnection.model("DeviceUseStatement_history").create(item);
        next();
    });

    DeviceUseStatementSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const DeviceUseStatementModel = modelConnection.model("DeviceUseStatement", DeviceUseStatementSchema, "DeviceUseStatement");
    return DeviceUseStatementModel;
};