...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = StructureDefinition;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const StructureDefinitionSchema = new mongoose.Schema(StructureDefinition, schemaConfig);


    StructureDefinitionSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    StructureDefinitionSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/StructureDefinition/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
    ...
    StructureDefinitionSchema.pre('findOneAndDelete', async function(next) {
