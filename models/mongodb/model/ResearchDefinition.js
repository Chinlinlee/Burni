...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = ResearchDefinition;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const ResearchDefinitionSchema = new mongoose.Schema(ResearchDefinition, schemaConfig);


    ResearchDefinitionSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    ResearchDefinitionSchema.pre('save', async function(next) {
        const mongodb = modelConnection;
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            let storedID = await mongodb.model("FHIRStoredID").findOne({
                id: this.id
            });
            if (storedID.resourceType != "ResearchDefinition") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("ResearchDefinition_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    ResearchDefinitionSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/ResearchDefinition/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("ResearchDefinition_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/ResearchDefinition/${item.id}/_history/${version}`
    ...
    ResearchDefinitionSchema.pre('findOneAndUpdate', async function(next) {
    ...
    ResearchDefinitionSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("ResearchDefinition_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    ResearchDefinitionSchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in ResearchDefinition resource`);
        }
        ...
        let createdDocs = await modelConnection.model("ResearchDefinition_history").create(item);
        next();
    });

    ResearchDefinitionSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const ResearchDefinitionModel = modelConnection.model("ResearchDefinition", ResearchDefinitionSchema, "ResearchDefinition");
    return ResearchDefinitionModel;
};