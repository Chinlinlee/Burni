...
module.exports = function() {
    ...
    AdverseEventSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.AdverseEvent_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    AdverseEventSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['AdverseEvent_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/AdverseEvent/${item.id}/_history/${version}`
    ...
    AdverseEventSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['AdverseEvent_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const AdverseEventModel = mongoose.model("AdverseEvent", AdverseEventSchema, "AdverseEvent");
