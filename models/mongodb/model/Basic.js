...
module.exports = function() {
    ...
    BasicSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.Basic_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    BasicSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['Basic_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Basic/${item.id}/_history/${version}`
    ...
    BasicSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['Basic_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const BasicModel = mongoose.model("Basic", BasicSchema, "Basic");
    return BasicModel;
};