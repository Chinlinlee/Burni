...
module.exports = function(connection = mongoose) {
    ...
    const BasicSchema = new mongoose.Schema(Basic, schemaConfig);


    BasicSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    BasicSchema.pre('save', async function(next) {
        ...
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            ...
            if (storedID.resourceType != "Basic") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("Basic_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    BasicSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Basic/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("Basic_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Basic/${item.id}/_history/${version}`
    ...
    BasicSchema.pre('findOneAndUpdate', async function(next) {
    ...
    BasicSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("Basic_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    BasicSchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in Basic resource`);
        }
        ...
        let createdDocs = await modelConnection.model("Basic_history").create(item);
        next();
    });

    BasicSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const BasicModel = modelConnection.model("Basic", BasicSchema, "Basic");
    return BasicModel;
};