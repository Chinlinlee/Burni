...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = RiskAssessment;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const RiskAssessmentSchema = new mongoose.Schema(RiskAssessment, schemaConfig);


    RiskAssessmentSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    RiskAssessmentSchema.pre('save', async function(next) {
        ...
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            ...
            if (storedID.resourceType != "RiskAssessment") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("RiskAssessment_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    RiskAssessmentSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            setHistoryProvenance(item, {
                method: "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/RiskAssessment/${item.id}/_history/${version}`,
                status: "201"
            });
            let createdDocs = await mongodb.model("RiskAssessment_history").create(item);
        } else {
            setHistoryProvenance(item, {
                method: "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/RiskAssessment/${item.id}/_history/${version}`,
    ...
    RiskAssessmentSchema.pre('findOneAndUpdate', async function(next) {
    ...
    RiskAssessmentSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("RiskAssessment_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    RiskAssessmentSchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in RiskAssessment resource`);
        }
        ...
        let createdDocs = await modelConnection.model("RiskAssessment_history").create(item);
        next();
    });

    RiskAssessmentSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const RiskAssessmentModel = modelConnection.model("RiskAssessment", RiskAssessmentSchema, "RiskAssessment");
