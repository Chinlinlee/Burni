...
module.exports = function() {
    ...
    RiskAssessmentSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.RiskAssessment_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    RiskAssessmentSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['RiskAssessment_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/RiskAssessment/${item.id}/_history/${version}`
    ...
    RiskAssessmentSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['RiskAssessment_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const RiskAssessmentModel = mongoose.model("RiskAssessment", RiskAssessmentSchema, "RiskAssessment");
