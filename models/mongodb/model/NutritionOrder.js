...
module.exports = function(connection = mongoose) {
    ...
    NutritionOrderSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.model("NutritionOrder_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    NutritionOrderSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb.model("NutritionOrder_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/NutritionOrder/${item.id}/_history/${version}`
    ...
    NutritionOrderSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await modelConnection.model("NutritionOrder_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    NutritionOrderSchema.pre('findOneAndDelete', async function(next) {
        ...
        let createdDocs = await modelConnection.model("NutritionOrder_history").create(item);
        next();
    });
    ...
    const NutritionOrderModel = modelConnection.model("NutritionOrder", NutritionOrderSchema, "NutritionOrder");
