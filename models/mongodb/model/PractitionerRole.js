...
module.exports = function() {
    ...
    PractitionerRoleSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.PractitionerRole_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    PractitionerRoleSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['PractitionerRole_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/PractitionerRole/${item.id}/_history/${version}`
    ...
    PractitionerRoleSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['PractitionerRole_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const PractitionerRoleModel = mongoose.model("PractitionerRole", PractitionerRoleSchema, "PractitionerRole");
