...
module.exports = function() {
    ...
    module.exports.schema = AppointmentResponse;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const AppointmentResponseSchema = new mongoose.Schema(AppointmentResponse, schemaConfig);


    AppointmentResponseSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    AppointmentResponseSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/AppointmentResponse/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
    ...
    AppointmentResponseSchema.pre('findOneAndDelete', async function(next) {
