...
module.exports = function(connection = mongoose) {
    ...
    SubstanceProteinSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.model("SubstanceProtein_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    SubstanceProteinSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb.model("SubstanceProtein_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/SubstanceProtein/${item.id}/_history/${version}`
    ...
    SubstanceProteinSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await modelConnection.model("SubstanceProtein_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    SubstanceProteinSchema.pre('findOneAndDelete', async function(next) {
        ...
        let createdDocs = await modelConnection.model("SubstanceProtein_history").create(item);
        next();
    });
    ...
    const SubstanceProteinModel = modelConnection.model("SubstanceProtein", SubstanceProteinSchema, "SubstanceProtein");
