...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = MedicationDispense;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const MedicationDispenseSchema = new mongoose.Schema(MedicationDispense, schemaConfig);


    MedicationDispenseSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    MedicationDispenseSchema.pre('save', async function(next) {
        ...
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            ...
            if (storedID.resourceType != "MedicationDispense") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("MedicationDispense_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    MedicationDispenseSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/MedicationDispense/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("MedicationDispense_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/MedicationDispense/${item.id}/_history/${version}`
    ...
    MedicationDispenseSchema.pre('findOneAndUpdate', async function(next) {
    ...
    MedicationDispenseSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("MedicationDispense_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    MedicationDispenseSchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in MedicationDispense resource`);
        }
        ...
        let createdDocs = await modelConnection.model("MedicationDispense_history").create(item);
        next();
    });

    MedicationDispenseSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const MedicationDispenseModel = modelConnection.model("MedicationDispense", MedicationDispenseSchema, "MedicationDispense");
    return MedicationDispenseModel;
};