...
module.exports = function(connection = mongoose) {
    ...
    HealthcareServiceSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.model("HealthcareService_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    HealthcareServiceSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb.model("HealthcareService_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/HealthcareService/${item.id}/_history/${version}`
    ...
    HealthcareServiceSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await modelConnection.model("HealthcareService_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    HealthcareServiceSchema.pre('findOneAndDelete', async function(next) {
        ...
        let createdDocs = await modelConnection.model("HealthcareService_history").create(item);
        next();
    });
    ...
    const HealthcareServiceModel = modelConnection.model("HealthcareService", HealthcareServiceSchema, "HealthcareService");
