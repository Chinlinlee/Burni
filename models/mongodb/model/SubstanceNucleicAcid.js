...
module.exports = function() {
    ...
    SubstanceNucleicAcidSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.SubstanceNucleicAcid_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    SubstanceNucleicAcidSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['SubstanceNucleicAcid_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/SubstanceNucleicAcid/${item.id}/_history/${version}`
    ...
    SubstanceNucleicAcidSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['SubstanceNucleicAcid_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const SubstanceNucleicAcidModel = mongoose.model("SubstanceNucleicAcid", SubstanceNucleicAcidSchema, "SubstanceNucleicAcid");
