...
module.exports = function(connection = mongoose) {
    ...
    CareTeamSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.model("CareTeam_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    CareTeamSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb.model("CareTeam_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/CareTeam/${item.id}/_history/${version}`
    ...
    CareTeamSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await modelConnection.model("CareTeam_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    CareTeamSchema.pre('findOneAndDelete', async function(next) {
        ...
        let createdDocs = await modelConnection.model("CareTeam_history").create(item);
        next();
    });
    ...
    const CareTeamModel = modelConnection.model("CareTeam", CareTeamSchema, "CareTeam");
