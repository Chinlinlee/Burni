...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = BodyStructure;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const BodyStructureSchema = new mongoose.Schema(BodyStructure, schemaConfig);


    BodyStructureSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    BodyStructureSchema.pre('save', async function(next) {
        const mongodb = modelConnection;
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            let storedID = await mongodb.model("FHIRStoredID").findOne({
                id: this.id
            });
            if (storedID.resourceType != "BodyStructure") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("BodyStructure_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    BodyStructureSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/BodyStructure/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("BodyStructure_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/BodyStructure/${item.id}/_history/${version}`
    ...
    BodyStructureSchema.pre('findOneAndUpdate', async function(next) {
    ...
    BodyStructureSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("BodyStructure_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    BodyStructureSchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in BodyStructure resource`);
        }
        ...
        let createdDocs = await modelConnection.model("BodyStructure_history").create(item);
        next();
    });

    BodyStructureSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const BodyStructureModel = modelConnection.model("BodyStructure", BodyStructureSchema, "BodyStructure");
    return BodyStructureModel;
};