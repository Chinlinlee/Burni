...
module.exports = function(connection = mongoose) {
    ...
    ImplementationGuideSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.model("ImplementationGuide_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    ImplementationGuideSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb.model("ImplementationGuide_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/ImplementationGuide/${item.id}/_history/${version}`
    ...
    ImplementationGuideSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await modelConnection.model("ImplementationGuide_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    ImplementationGuideSchema.pre('findOneAndDelete', async function(next) {
        ...
        let createdDocs = await modelConnection.model("ImplementationGuide_history").create(item);
        next();
    });
    ...
    const ImplementationGuideModel = modelConnection.model("ImplementationGuide", ImplementationGuideSchema, "ImplementationGuide");
