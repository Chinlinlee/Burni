...
module.exports = function() {
    ...
    module.exports.schema = Coverage;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const CoverageSchema = new mongoose.Schema(Coverage, schemaConfig);


    CoverageSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    CoverageSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Coverage/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
    ...
    CoverageSchema.pre('findOneAndDelete', async function(next) {
