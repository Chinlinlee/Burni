...
module.exports = function() {
    ...
    ClinicalImpressionSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.ClinicalImpression_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    ClinicalImpressionSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['ClinicalImpression_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/ClinicalImpression/${item.id}/_history/${version}`
    ...
    ClinicalImpressionSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['ClinicalImpression_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const ClinicalImpressionModel = mongoose.model("ClinicalImpression", ClinicalImpressionSchema, "ClinicalImpression");
