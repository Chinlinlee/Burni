...
module.exports = function() {
    ...
    MedicinalProductSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.MedicinalProduct_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    MedicinalProductSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['MedicinalProduct_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/MedicinalProduct/${item.id}/_history/${version}`
    ...
    MedicinalProductSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['MedicinalProduct_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const MedicinalProductModel = mongoose.model("MedicinalProduct", MedicinalProductSchema, "MedicinalProduct");
