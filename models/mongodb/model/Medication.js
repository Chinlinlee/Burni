...
module.exports = function() {
    ...
    MedicationSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.Medication_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    MedicationSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['Medication_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Medication/${item.id}/_history/${version}`
    ...
    MedicationSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['Medication_history'].create(item);
        } catch (e) {
            console.error(e);
        }
