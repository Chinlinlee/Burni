...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = Medication;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const MedicationSchema = new mongoose.Schema(Medication, schemaConfig);


    MedicationSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    MedicationSchema.pre('save', async function(next) {
        ...
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            ...
            if (storedID.resourceType != "Medication") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("Medication_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    MedicationSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Medication/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("Medication_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Medication/${item.id}/_history/${version}`
    ...
    MedicationSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("Medication_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    MedicationSchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in Medication resource`);
        }
        ...
        let createdDocs = await modelConnection.model("Medication_history").create(item);
        next();
    });

    MedicationSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const MedicationModel = modelConnection.model("Medication", MedicationSchema, "Medication");
