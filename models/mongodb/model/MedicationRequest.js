...
module.exports = function(connection = mongoose) {
    ...
    MedicationRequestSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.model("MedicationRequest_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    MedicationRequestSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb.model("MedicationRequest_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/MedicationRequest/${item.id}/_history/${version}`
    ...
    MedicationRequestSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await modelConnection.model("MedicationRequest_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    MedicationRequestSchema.pre('findOneAndDelete', async function(next) {
        ...
        let createdDocs = await modelConnection.model("MedicationRequest_history").create(item);
        next();
    });
    ...
    const MedicationRequestModel = modelConnection.model("MedicationRequest", MedicationRequestSchema, "MedicationRequest");
