...
module.exports = function(connection = mongoose) {
    ...
    ResearchStudySchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.model("ResearchStudy_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    ResearchStudySchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb.model("ResearchStudy_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/ResearchStudy/${item.id}/_history/${version}`
    ...
    ResearchStudySchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await modelConnection.model("ResearchStudy_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    ResearchStudySchema.pre('findOneAndDelete', async function(next) {
        ...
        let createdDocs = await modelConnection.model("ResearchStudy_history").create(item);
        next();
    });
    ...
    const ResearchStudyModel = modelConnection.model("ResearchStudy", ResearchStudySchema, "ResearchStudy");
