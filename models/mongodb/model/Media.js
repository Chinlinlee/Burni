...
module.exports = function(connection = mongoose) {
    ...
    MediaSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.model("Media_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    MediaSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb.model("Media_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Media/${item.id}/_history/${version}`
    ...
    MediaSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await modelConnection.model("Media_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    MediaSchema.pre('findOneAndDelete', async function(next) {
        ...
        let createdDocs = await modelConnection.model("Media_history").create(item);
        next();
    });
    ...
    const MediaModel = modelConnection.model("Media", MediaSchema, "Media");
    return MediaModel;
};