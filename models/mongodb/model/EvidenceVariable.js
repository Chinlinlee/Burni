...
module.exports = function() {
    ...
    EvidenceVariableSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.EvidenceVariable_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    EvidenceVariableSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['EvidenceVariable_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/EvidenceVariable/${item.id}/_history/${version}`
    ...
    EvidenceVariableSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['EvidenceVariable_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const EvidenceVariableModel = mongoose.model("EvidenceVariable", EvidenceVariableSchema, "EvidenceVariable");
