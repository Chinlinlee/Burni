...
module.exports = function() {
    ...
    CompositionSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.Composition_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    CompositionSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['Composition_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Composition/${item.id}/_history/${version}`
    ...
    CompositionSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['Composition_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const CompositionModel = mongoose.model("Composition", CompositionSchema, "Composition");
