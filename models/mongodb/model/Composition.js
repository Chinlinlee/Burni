...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = Composition;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const CompositionSchema = new mongoose.Schema(Composition, schemaConfig);


    CompositionSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    CompositionSchema.pre('save', async function(next) {
        ...
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            ...
            if (storedID.resourceType != "Composition") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("Composition_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    CompositionSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Composition/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("Composition_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Composition/${item.id}/_history/${version}`
    ...
    CompositionSchema.pre('findOneAndUpdate', async function(next) {
    ...
    CompositionSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("Composition_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    CompositionSchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in Composition resource`);
        }
        ...
        let createdDocs = await modelConnection.model("Composition_history").create(item);
        next();
    });

    CompositionSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const CompositionModel = modelConnection.model("Composition", CompositionSchema, "Composition");
