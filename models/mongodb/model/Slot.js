...
module.exports = function(connection = mongoose) {
    ...
    const SlotSchema = new mongoose.Schema(Slot, schemaConfig);


    SlotSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    SlotSchema.pre('save', async function(next) {
        ...
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            ...
            if (storedID.resourceType != "Slot") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("Slot_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    SlotSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Slot/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("Slot_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Slot/${item.id}/_history/${version}`
    ...
    SlotSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("Slot_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    SlotSchema.pre('findOneAndDelete', async function(next) {
        ...
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in Slot resource`);
        }
        ...
        let createdDocs = await modelConnection.model("Slot_history").create(item);
        next();
    });

    SlotSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const SlotModel = modelConnection.model("Slot", SlotSchema, "Slot");
    return SlotModel;
};