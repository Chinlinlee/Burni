...
module.exports = function() {
    ...
    SlotSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.Slot_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    SlotSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['Slot_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Slot/${item.id}/_history/${version}`
    ...
    SlotSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['Slot_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const SlotModel = mongoose.model("Slot", SlotSchema, "Slot");
    return SlotModel;
};