...
module.exports = function() {
    ...
    SubstanceSourceMaterialSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.SubstanceSourceMaterial_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    SubstanceSourceMaterialSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['SubstanceSourceMaterial_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/SubstanceSourceMaterial/${item.id}/_history/${version}`
    ...
    SubstanceSourceMaterialSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['SubstanceSourceMaterial_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const SubstanceSourceMaterialModel = mongoose.model("SubstanceSourceMaterial", SubstanceSourceMaterialSchema, "SubstanceSourceMaterial");
