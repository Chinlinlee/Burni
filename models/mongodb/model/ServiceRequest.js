...
module.exports = function() {
    ...
    ServiceRequestSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.ServiceRequest_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    ServiceRequestSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['ServiceRequest_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/ServiceRequest/${item.id}/_history/${version}`
    ...
    ServiceRequestSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['ServiceRequest_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const ServiceRequestModel = mongoose.model("ServiceRequest", ServiceRequestSchema, "ServiceRequest");
