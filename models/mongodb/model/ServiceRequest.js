...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = ServiceRequest;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const ServiceRequestSchema = new mongoose.Schema(ServiceRequest, schemaConfig);


    ServiceRequestSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    ServiceRequestSchema.pre('save', async function(next) {
        ...
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            ...
            if (storedID.resourceType != "ServiceRequest") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("ServiceRequest_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    ServiceRequestSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/ServiceRequest/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("ServiceRequest_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/ServiceRequest/${item.id}/_history/${version}`
    ...
    ServiceRequestSchema.pre('findOneAndUpdate', async function(next) {
    ...
    ServiceRequestSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("ServiceRequest_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    ServiceRequestSchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in ServiceRequest resource`);
        }
        ...
        let createdDocs = await modelConnection.model("ServiceRequest_history").create(item);
        next();
    });

    ServiceRequestSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const ServiceRequestModel = modelConnection.model("ServiceRequest", ServiceRequestSchema, "ServiceRequest");
