...
module.exports = function() {
    ...
    ListSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.List_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    ListSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['List_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/List/${item.id}/_history/${version}`
    ...
    ListSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['List_history'].create(item);
        } catch (e) {
            console.error(e);
        }
