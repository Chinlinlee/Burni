...
module.exports = function(connection = mongoose) {
    ...
    const ListSchema = new mongoose.Schema(List, schemaConfig);


    ListSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    ListSchema.pre('save', async function(next) {
        ...
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            ...
            if (storedID.resourceType != "List") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("List_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    ListSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/List/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("List_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/List/${item.id}/_history/${version}`
    ...
    ListSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("List_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    ListSchema.pre('findOneAndDelete', async function(next) {
        ...
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in List resource`);
        }
        ...
        let createdDocs = await modelConnection.model("List_history").create(item);
        next();
    });

    ListSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const ListModel = modelConnection.model("List", ListSchema, "List");
    return ListModel;
};