...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = Location;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const LocationSchema = new mongoose.Schema(Location, schemaConfig);


    LocationSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    LocationSchema.pre('save', async function(next) {
        ...
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            ...
            if (storedID.resourceType != "Location") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("Location_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    LocationSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Location/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("Location_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Location/${item.id}/_history/${version}`
    ...
    LocationSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("Location_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    LocationSchema.pre('findOneAndDelete', async function(next) {
        ...
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in Location resource`);
        }
        ...
        let createdDocs = await modelConnection.model("Location_history").create(item);
        next();
    });

    LocationSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const LocationModel = modelConnection.model("Location", LocationSchema, "Location");
