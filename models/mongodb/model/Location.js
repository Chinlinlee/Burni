...
module.exports = function() {
    ...
    LocationSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.Location_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    LocationSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['Location_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Location/${item.id}/_history/${version}`
    ...
    LocationSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['Location_history'].create(item);
        } catch (e) {
            console.error(e);
        }
