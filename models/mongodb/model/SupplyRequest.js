...
module.exports = function(connection = mongoose) {
    ...
    SupplyRequestSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.model("SupplyRequest_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    SupplyRequestSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb.model("SupplyRequest_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/SupplyRequest/${item.id}/_history/${version}`
    ...
    SupplyRequestSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await modelConnection.model("SupplyRequest_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    SupplyRequestSchema.pre('findOneAndDelete', async function(next) {
        ...
        let createdDocs = await modelConnection.model("SupplyRequest_history").create(item);
        next();
    });
    ...
    const SupplyRequestModel = modelConnection.model("SupplyRequest", SupplyRequestSchema, "SupplyRequest");
