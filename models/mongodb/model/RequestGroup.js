...
module.exports = function() {
    ...
    RequestGroupSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.RequestGroup_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    RequestGroupSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['RequestGroup_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/RequestGroup/${item.id}/_history/${version}`
    ...
    RequestGroupSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['RequestGroup_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const RequestGroupModel = mongoose.model("RequestGroup", RequestGroupSchema, "RequestGroup");
