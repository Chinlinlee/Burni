...
module.exports = function(connection = mongoose) {
    ...
    MedicationStatementSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.model("MedicationStatement_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    MedicationStatementSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb.model("MedicationStatement_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/MedicationStatement/${item.id}/_history/${version}`
    ...
    MedicationStatementSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await modelConnection.model("MedicationStatement_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    MedicationStatementSchema.pre('findOneAndDelete', async function(next) {
        ...
        let createdDocs = await modelConnection.model("MedicationStatement_history").create(item);
        next();
    });
    ...
    const MedicationStatementModel = modelConnection.model("MedicationStatement", MedicationStatementSchema, "MedicationStatement");
