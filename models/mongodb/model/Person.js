...
module.exports = function() {
    ...
    PersonSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.Person_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    PersonSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['Person_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Person/${item.id}/_history/${version}`
    ...
    PersonSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['Person_history'].create(item);
        } catch (e) {
            console.error(e);
        }
