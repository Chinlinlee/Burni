...
module.exports = function(connection = mongoose) {
    ...
    const PersonSchema = new mongoose.Schema(Person, schemaConfig);


    PersonSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    PersonSchema.pre('save', async function(next) {
        ...
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            ...
            if (storedID.resourceType != "Person") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("Person_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    PersonSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Person/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("Person_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Person/${item.id}/_history/${version}`
    ...
    PersonSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("Person_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    PersonSchema.pre('findOneAndDelete', async function(next) {
        ...
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in Person resource`);
        }
        ...
        let createdDocs = await modelConnection.model("Person_history").create(item);
        next();
    });

    PersonSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const PersonModel = modelConnection.model("Person", PersonSchema, "Person");
    return PersonModel;
};