...
module.exports = function() {
    ...
    TestScriptSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.TestScript_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    TestScriptSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['TestScript_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/TestScript/${item.id}/_history/${version}`
    ...
    TestScriptSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['TestScript_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const TestScriptModel = mongoose.model("TestScript", TestScriptSchema, "TestScript");
