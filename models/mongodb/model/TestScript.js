...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = TestScript;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const TestScriptSchema = new mongoose.Schema(TestScript, schemaConfig);


    TestScriptSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    TestScriptSchema.pre('save', async function(next) {
        const mongodb = modelConnection;
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            let storedID = await mongodb.model("FHIRStoredID").findOne({
                id: this.id
            });
            if (storedID.resourceType != "TestScript") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("TestScript_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    TestScriptSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/TestScript/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("TestScript_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/TestScript/${item.id}/_history/${version}`
    ...
    TestScriptSchema.pre('findOneAndUpdate', async function(next) {
    ...
    TestScriptSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("TestScript_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    TestScriptSchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in TestScript resource`);
        }
        ...
        let createdDocs = await modelConnection.model("TestScript_history").create(item);
        next();
    });

    TestScriptSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const TestScriptModel = modelConnection.model("TestScript", TestScriptSchema, "TestScript");
