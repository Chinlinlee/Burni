...
module.exports = function(connection = mongoose) {
    ...
    MessageHeaderSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.model("MessageHeader_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    MessageHeaderSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb.model("MessageHeader_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/MessageHeader/${item.id}/_history/${version}`
    ...
    MessageHeaderSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await modelConnection.model("MessageHeader_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    MessageHeaderSchema.pre('findOneAndDelete', async function(next) {
        ...
        let createdDocs = await modelConnection.model("MessageHeader_history").create(item);
        next();
    });
    ...
    const MessageHeaderModel = modelConnection.model("MessageHeader", MessageHeaderSchema, "MessageHeader");
