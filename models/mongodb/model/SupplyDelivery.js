...
module.exports = function() {
    ...
    SupplyDeliverySchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.SupplyDelivery_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    SupplyDeliverySchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['SupplyDelivery_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/SupplyDelivery/${item.id}/_history/${version}`
    ...
    SupplyDeliverySchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['SupplyDelivery_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const SupplyDeliveryModel = mongoose.model("SupplyDelivery", SupplyDeliverySchema, "SupplyDelivery");
