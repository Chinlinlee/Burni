...
module.exports = function() {
    ...
    DeviceMetricSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.DeviceMetric_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    DeviceMetricSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['DeviceMetric_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/DeviceMetric/${item.id}/_history/${version}`
    ...
    DeviceMetricSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['DeviceMetric_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const DeviceMetricModel = mongoose.model("DeviceMetric", DeviceMetricSchema, "DeviceMetric");
