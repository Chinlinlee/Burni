...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = CompartmentDefinition;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const CompartmentDefinitionSchema = new mongoose.Schema(CompartmentDefinition, schemaConfig);


    CompartmentDefinitionSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    CompartmentDefinitionSchema.pre('save', async function(next) {
        const mongodb = modelConnection;
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            let storedID = await mongodb.model("FHIRStoredID").findOne({
                id: this.id
            });
            if (storedID.resourceType != "CompartmentDefinition") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("CompartmentDefinition_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    CompartmentDefinitionSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/CompartmentDefinition/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("CompartmentDefinition_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/CompartmentDefinition/${item.id}/_history/${version}`
    ...
    CompartmentDefinitionSchema.pre('findOneAndUpdate', async function(next) {
    ...
    CompartmentDefinitionSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("CompartmentDefinition_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    CompartmentDefinitionSchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in CompartmentDefinition resource`);
        }
        ...
        let createdDocs = await modelConnection.model("CompartmentDefinition_history").create(item);
        next();
    });

    CompartmentDefinitionSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const CompartmentDefinitionModel = modelConnection.model("CompartmentDefinition", CompartmentDefinitionSchema, "CompartmentDefinition");
    return CompartmentDefinitionModel;
};