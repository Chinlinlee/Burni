...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = Evidence;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const EvidenceSchema = new mongoose.Schema(Evidence, schemaConfig);


    EvidenceSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    EvidenceSchema.pre('save', async function(next) {
        ...
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            ...
            if (storedID.resourceType != "Evidence") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("Evidence_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    EvidenceSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Evidence/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("Evidence_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Evidence/${item.id}/_history/${version}`
    ...
    EvidenceSchema.pre('findOneAndUpdate', async function(next) {
    ...
    EvidenceSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("Evidence_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    EvidenceSchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in Evidence resource`);
        }
        ...
        let createdDocs = await modelConnection.model("Evidence_history").create(item);
        next();
    });

    EvidenceSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const EvidenceModel = modelConnection.model("Evidence", EvidenceSchema, "Evidence");
