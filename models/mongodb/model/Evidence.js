...
module.exports = function() {
    ...
    EvidenceSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.Evidence_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    EvidenceSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['Evidence_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Evidence/${item.id}/_history/${version}`
    ...
    EvidenceSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['Evidence_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const EvidenceModel = mongoose.model("Evidence", EvidenceSchema, "Evidence");
