...
module.exports = function() {
    ...
    ProvenanceSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.Provenance_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    ProvenanceSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['Provenance_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Provenance/${item.id}/_history/${version}`
    ...
    ProvenanceSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['Provenance_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const ProvenanceModel = mongoose.model("Provenance", ProvenanceSchema, "Provenance");
