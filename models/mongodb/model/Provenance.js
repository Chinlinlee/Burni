...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = Provenance;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const ProvenanceSchema = new mongoose.Schema(Provenance, schemaConfig);


    ProvenanceSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    ProvenanceSchema.pre('save', async function(next) {
        ...
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            ...
            if (storedID.resourceType != "Provenance") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("Provenance_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    ProvenanceSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Provenance/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("Provenance_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Provenance/${item.id}/_history/${version}`
    ...
    ProvenanceSchema.pre('findOneAndUpdate', async function(next) {
    ...
    ProvenanceSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("Provenance_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    ProvenanceSchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in Provenance resource`);
        }
        ...
        let createdDocs = await modelConnection.model("Provenance_history").create(item);
        next();
    });

    ProvenanceSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const ProvenanceModel = modelConnection.model("Provenance", ProvenanceSchema, "Provenance");
