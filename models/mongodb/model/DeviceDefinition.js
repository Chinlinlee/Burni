...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = DeviceDefinition;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const DeviceDefinitionSchema = new mongoose.Schema(DeviceDefinition, schemaConfig);


    DeviceDefinitionSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    DeviceDefinitionSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/DeviceDefinition/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
    ...
    DeviceDefinitionSchema.pre('findOneAndDelete', async function(next) {
