...
module.exports = function() {
    ...
    ImagingStudySchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.ImagingStudy_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    ImagingStudySchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['ImagingStudy_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/ImagingStudy/${item.id}/_history/${version}`
    ...
    ImagingStudySchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['ImagingStudy_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const ImagingStudyModel = mongoose.model("ImagingStudy", ImagingStudySchema, "ImagingStudy");
