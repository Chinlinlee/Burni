...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = ImagingStudy;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const ImagingStudySchema = new mongoose.Schema(ImagingStudy, schemaConfig);
    ...
    ImagingStudySchema.pre('save', async function(next) {
        ...
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            ...
            if (storedID.resourceType != "ImagingStudy") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("ImagingStudy_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    ImagingStudySchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/ImagingStudy/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("ImagingStudy_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/ImagingStudy/${item.id}/_history/${version}`
    ...
    ImagingStudySchema.pre('findOneAndUpdate', async function(next) {
    ...
    ImagingStudySchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("ImagingStudy_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    ImagingStudySchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in ImagingStudy resource`);
        }
        ...
        let createdDocs = await modelConnection.model("ImagingStudy_history").create(item);
        next();
    });

    ImagingStudySchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const ImagingStudyModel = modelConnection.model("ImagingStudy", ImagingStudySchema, "ImagingStudy");
