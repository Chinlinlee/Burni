...
module.exports = function(connection = mongoose) {
    ...
    CapabilityStatementSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.model("CapabilityStatement_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    CapabilityStatementSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb.model("CapabilityStatement_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/CapabilityStatement/${item.id}/_history/${version}`
    ...
    CapabilityStatementSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await modelConnection.model("CapabilityStatement_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    CapabilityStatementSchema.pre('findOneAndDelete', async function(next) {
        ...
        let createdDocs = await modelConnection.model("CapabilityStatement_history").create(item);
        next();
    });
    ...
    const CapabilityStatementModel = modelConnection.model("CapabilityStatement", CapabilityStatementSchema, "CapabilityStatement");
