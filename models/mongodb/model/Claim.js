...
module.exports = function(connection = mongoose) {
    ...
    const ClaimSchema = new mongoose.Schema(Claim, schemaConfig);


    ClaimSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    ClaimSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Claim/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
    ...
    ClaimSchema.pre('findOneAndDelete', async function(next) {
