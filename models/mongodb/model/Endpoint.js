...
module.exports = function() {
    ...
    EndpointSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.Endpoint_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    EndpointSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['Endpoint_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Endpoint/${item.id}/_history/${version}`
    ...
    EndpointSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['Endpoint_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const EndpointModel = mongoose.model("Endpoint", EndpointSchema, "Endpoint");
