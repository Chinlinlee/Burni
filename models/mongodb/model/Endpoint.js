...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = Endpoint;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const EndpointSchema = new mongoose.Schema(Endpoint, schemaConfig);
    ...
    EndpointSchema.pre('save', async function(next) {
        ...
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            ...
            if (storedID.resourceType != "Endpoint") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("Endpoint_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    EndpointSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Endpoint/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("Endpoint_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Endpoint/${item.id}/_history/${version}`
    ...
    EndpointSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("Endpoint_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    EndpointSchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in Endpoint resource`);
        }
        ...
        let createdDocs = await modelConnection.model("Endpoint_history").create(item);
        next();
    });

    EndpointSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const EndpointModel = modelConnection.model("Endpoint", EndpointSchema, "Endpoint");
