...
module.exports = function(connection = mongoose) {
    ...
    const LibrarySchema = new mongoose.Schema(Library, schemaConfig);


    LibrarySchema.methods.getFHIRField = function() {
        let result = this;
    ...
    LibrarySchema.pre('save', async function(next) {
        ...
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            ...
            if (storedID.resourceType != "Library") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("Library_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    LibrarySchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Library/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("Library_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Library/${item.id}/_history/${version}`
    ...
    LibrarySchema.pre('findOneAndUpdate', async function(next) {
    ...
    LibrarySchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("Library_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    LibrarySchema.pre('findOneAndDelete', async function(next) {
        ...
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in Library resource`);
        }
        ...
        let createdDocs = await modelConnection.model("Library_history").create(item);
        next();
    });

    LibrarySchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const LibraryModel = modelConnection.model("Library", LibrarySchema, "Library");
    return LibraryModel;
};