...
module.exports = function() {
    ...
    LibrarySchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.Library_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    LibrarySchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['Library_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Library/${item.id}/_history/${version}`
    ...
    LibrarySchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['Library_history'].create(item);
        } catch (e) {
            console.error(e);
        }
