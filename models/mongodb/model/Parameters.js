...
module.exports = function(connection = mongoose) {
    ...
    ParametersSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.model("Parameters_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    ParametersSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb.model("Parameters_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Parameters/${item.id}/_history/${version}`
    ...
    ParametersSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await modelConnection.model("Parameters_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    ParametersSchema.pre('findOneAndDelete', async function(next) {
        ...
        let createdDocs = await modelConnection.model("Parameters_history").create(item);
        next();
    });
    ...
    const ParametersModel = modelConnection.model("Parameters", ParametersSchema, "Parameters");
