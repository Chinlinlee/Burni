...
module.exports = function() {
    ...
    module.exports.schema = Specimen;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const SpecimenSchema = new mongoose.Schema(Specimen, schemaConfig);


    SpecimenSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    SpecimenSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Specimen/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
    ...
    SpecimenSchema.pre('findOneAndDelete', async function(next) {
