...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = CommunicationRequest;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const CommunicationRequestSchema = new mongoose.Schema(CommunicationRequest, schemaConfig);


    CommunicationRequestSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    CommunicationRequestSchema.pre('save', async function(next) {
        const mongodb = modelConnection;
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            let storedID = await mongodb.model("FHIRStoredID").findOne({
                id: this.id
            });
            if (storedID.resourceType != "CommunicationRequest") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("CommunicationRequest_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    CommunicationRequestSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/CommunicationRequest/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("CommunicationRequest_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/CommunicationRequest/${item.id}/_history/${version}`
    ...
    CommunicationRequestSchema.pre('findOneAndUpdate', async function(next) {
    ...
    CommunicationRequestSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("CommunicationRequest_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    CommunicationRequestSchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in CommunicationRequest resource`);
        }
        ...
        let createdDocs = await modelConnection.model("CommunicationRequest_history").create(item);
        next();
    });

    CommunicationRequestSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const CommunicationRequestModel = modelConnection.model("CommunicationRequest", CommunicationRequestSchema, "CommunicationRequest");
