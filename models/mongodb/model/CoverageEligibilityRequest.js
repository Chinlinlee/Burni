...
module.exports = function() {
    ...
    module.exports.schema = CoverageEligibilityRequest;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const CoverageEligibilityRequestSchema = new mongoose.Schema(CoverageEligibilityRequest, schemaConfig);


    CoverageEligibilityRequestSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    CoverageEligibilityRequestSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/CoverageEligibilityRequest/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
    ...
    CoverageEligibilityRequestSchema.pre('findOneAndDelete', async function(next) {
