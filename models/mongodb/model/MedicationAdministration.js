...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = MedicationAdministration;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const MedicationAdministrationSchema = new mongoose.Schema(MedicationAdministration, schemaConfig);


    MedicationAdministrationSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    MedicationAdministrationSchema.pre('save', async function(next) {
        ...
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            ...
            if (storedID.resourceType != "MedicationAdministration") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("MedicationAdministration_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    MedicationAdministrationSchema.post('save', async function(result) {
        const mongodb = modelConnection;
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/MedicationAdministration/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
            let createdDocs = await mongodb.model("MedicationAdministration_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/MedicationAdministration/${item.id}/_history/${version}`
    ...
    MedicationAdministrationSchema.pre('findOneAndUpdate', async function(next) {
    ...
    MedicationAdministrationSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("MedicationAdministration_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    MedicationAdministrationSchema.pre('findOneAndDelete', async function(next) {
        const docToDelete = await this.model.findOne(this.getFilter());
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in MedicationAdministration resource`);
        }
        ...
        let createdDocs = await modelConnection.model("MedicationAdministration_history").create(item);
        next();
    });

    MedicationAdministrationSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const MedicationAdministrationModel = modelConnection.model("MedicationAdministration", MedicationAdministrationSchema, "MedicationAdministration");
