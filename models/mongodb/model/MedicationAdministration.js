...
module.exports = function() {
    ...
    MedicationAdministrationSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.MedicationAdministration_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    MedicationAdministrationSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['MedicationAdministration_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/MedicationAdministration/${item.id}/_history/${version}`
    ...
    MedicationAdministrationSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['MedicationAdministration_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const MedicationAdministrationModel = mongoose.model("MedicationAdministration", MedicationAdministrationSchema, "MedicationAdministration");
