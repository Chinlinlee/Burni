...
module.exports = function(connection = mongoose) {
    ...
    ActivityDefinitionSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.model("ActivityDefinition_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    ActivityDefinitionSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb.model("ActivityDefinition_history").create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/ActivityDefinition/${item.id}/_history/${version}`
    ...
    ActivityDefinitionSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await modelConnection.model("ActivityDefinition_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    ActivityDefinitionSchema.pre('findOneAndDelete', async function(next) {
        ...
        let createdDocs = await modelConnection.model("ActivityDefinition_history").create(item);
        next();
    });
    ...
    const ActivityDefinitionModel = modelConnection.model("ActivityDefinition", ActivityDefinitionSchema, "ActivityDefinition");
