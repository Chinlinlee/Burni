...
module.exports = function() {
    ...
    module.exports.schema = ObservationDefinition;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const ObservationDefinitionSchema = new mongoose.Schema(ObservationDefinition, schemaConfig);


    ObservationDefinitionSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    ObservationDefinitionSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            _.set(item, "request", {
                "method": "POST",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/ObservationDefinition/${item.id}/_history/${version}`
            });
            _.set(item, "response", {
                status: "201"
            });
    ...
    ObservationDefinitionSchema.pre('findOneAndDelete', async function(next) {
