...
module.exports = function(connection = mongoose) {
    ...
    module.exports.schema = Schedule;
    let schemaConfig = {
        toObject: {
            getters: true
        },
    ...
    const ScheduleSchema = new mongoose.Schema(Schedule, schemaConfig);


    ScheduleSchema.methods.getFHIRField = function() {
        let result = this;
    ...
    ScheduleSchema.pre('save', async function(next) {
        ...
        if (process.env.ENABLE_CHECK_ALL_RESOURCE_ID == "true") {
            ...
            if (storedID.resourceType != "Schedule") {
                console.error('err', storedID);
                return next(new Error(`The id->${this.id} stored by resource ${storedID.resourceType}`));
            }
        }

        const docInHistory = await mongodb.model("Schedule_history").findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    ScheduleSchema.post('save', async function(result) {
        const mongodb = modelConnection;
    ...
    ScheduleSchema.post('findOneAndUpdate', async function(result) {
        let item;
        if (result.value) {
        ...
        try {
            let history = await modelConnection.model("Schedule_history").create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    ScheduleSchema.pre('findOneAndDelete', async function(next) {
        ...
        if (!docToDelete) {
            next(`The id->${this.getFilter().id} not found in Schedule resource`);
        }
    ...
    ScheduleSchema.post('findOneAndDelete', async function(resource) {
        await updateRefBy(resource, modelConnection);
        await deleteEmptyRefBy(modelConnection);
    });

    const ScheduleModel = modelConnection.model("Schedule", ScheduleSchema, "Schedule");
