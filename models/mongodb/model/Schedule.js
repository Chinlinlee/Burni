...
module.exports = function() {
    ...
    ScheduleSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.Schedule_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    ScheduleSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['Schedule_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/Schedule/${item.id}/_history/${version}`
    ...
    ScheduleSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['Schedule_history'].create(item);
        } catch (e) {
            console.error(e);
        }
