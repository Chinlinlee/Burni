...
module.exports = function() {
    ...
    MolecularSequenceSchema.pre('save', async function(next) {
        ...
        const docInHistory = await mongodb.MolecularSequence_history.findOne({
                id: this.id
            })
            .sort({
                "meta.versionId": -1
    ...
    MolecularSequenceSchema.post('save', async function(result) {
        ...
        if (version == "1") {
            ...
            let createdDocs = await mongodb['MolecularSequence_history'].create(item);
        } else {
            _.set(item, "request", {
                "method": "PUT",
                url: `http://${process.env.FHIRSERVER_HOST}${port}/${process.env.FHIRSERVER_APIPATH}/MolecularSequence/${item.id}/_history/${version}`
    ...
    MolecularSequenceSchema.post('findOneAndUpdate', async function(result) {
        ...
        try {
            let history = await mongodb['MolecularSequence_history'].create(item);
        } catch (e) {
            console.error(e);
        }
    ...
    const MolecularSequenceModel = mongoose.model("MolecularSequence", MolecularSequenceSchema, "MolecularSequence");
