...
## MongoDB schema and index provisioning
...
The desired collection and index manifest is generated deterministically at runtime from registered models, the model catalog, and approved built-in temporal definitions. Provisioning reconciles MongoDB against that generated manifest, persists results in `MongoProvisioningState`, and uses `MongoProvisioningLock` with a database-scoped lease for multi-instance runs. Use `writeDesiredManifestArtifact()` when you need a persisted JSON copy (default path `models/mongodb/provisioning/artifacts/desired-index-manifest.json`).
...
This command runs control-plane initialization, acquires the lock, creates collections, creates baseline indexes, creates approved built-in temporal derived indexes, runs verify, and persists the manifest checksum and drift summary. Exit code `0` on success; any failed phase returns a non-zero status.
...
By default, `node server.js` does **not** run provisioning. Application readiness (`mongodb.ready`) and provisioning readiness are separate. Missing performance indexes do not block HTTP listen, but queries may fall back to COLLSCAN.
...
### Relationship to SearchParameter derived indexes

First-phase derived indexes cover approved built-in temporal SearchParameters only. Custom database SearchParameters remain executable but do not create derived indexes by default. See [Search parameters overview](/docs/search-parameter/overview).
