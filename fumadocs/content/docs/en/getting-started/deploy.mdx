...
The desired collection and index state is defined in the committed artifact at `models/mongodb/provisioning/artifacts/desired-index-manifest.json`. Provisioning creates missing collections and indexes from that manifest, persists results in `MongoProvisioningState`, and uses `MongoProvisioningLock` with a database-scoped lease for multi-instance runs.
